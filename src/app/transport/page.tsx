...
import { Container, Card, Table, Button, Form, Row, Col, Modal, Tabs, Tab } from 'react-bootstrap';
...
export default function TransportPage() {
    ...
    return (
        <Container fluid>
            <PageHeader title="Transport" subtitle="Manage buses, routes and drivers" />
            ...
            <Tabs defaultActiveKey="buses" className="mb-4">
                <Tab eventKey="buses" title={<span><FaBus className="me-2" />Buses & Routes</span>}>
                    <Card className="app-card">
                        <Card.Header className="d-flex justify-content-between align-items-center py-3">
                            <h5 className="mb-0">Bus Fleet</h5>
                            <Button variant="primary" size="sm" onClick={() => setShowBusModal(true)}>
                                <FaPlus className="me-2" /> Add Bus
                            </Button>
                        </Card.Header>
                        <Card.Body className="p-0">
                            <Table hover className="app-table mb-0">
                                <thead>
                                    <tr>
                                    ...
                                    {filteredBuses.map(bus => (
                                        <tr key={bus.id}>
                                            ...
                                            <td className="text-end pe-4">
                                                <Button variant="outline-primary" size="sm" className="me-2" onClick={() => handleViewPassengers(bus.id, bus.bus_number)}>
                                                    Passengers
                                                </Button>
                ...
                <Tab eventKey="drivers" title={<span><FaUserTie className="me-2" />Drivers</span>}>
                    <Card className="app-card">
                        <Card.Header className="d-flex justify-content-between align-items-center py-3">
                            <h5 className="mb-0">Driver Fleet</h5>
                            <Button variant="primary" size="sm" onClick={handleCreateDriverClick}>
                                ...
                                <tbody>
                                    {filteredDrivers.map(driver => (
                                        <tr key={driver.id}>
                                            <td className="ps-4">
                                                {driver.avatar_url ? (
                                                    <img src={driver.avatar_url} alt="" className="table-avatar" />
                                                ) : (
                                                    <div className="table-avatar-placeholder">N/A</div>
                                                )}
                                            </td>
                                            <td className="fw-bold">
                                                {driver.full_name}
                                                <div className="small text-muted">{driver.parentage ? `S/O ${driver.parentage}` : ''}</div>
                                            </td>
                                            ...
                                            <td className="text-end pe-4">
                                                <Button variant="outline-primary" size="sm" className="me-2" onClick={() => handleEditDriver(driver)}>Edit</Button>
            ...
            <Modal show={showBusModal} onHide={() => setShowBusModal(false)} centered>
                <Modal.Header closeButton><Modal.Title>Add Bus</Modal.Title></Modal.Header>
                <Modal.Body>
                    <Row>
                        <Col md={6}>
                            <Form.Group className="mb-3">
                                <Form.Label>Bus Number *</Form.Label>
                                <Form.Control placeholder="e.g. 01"
                        ...
                        <Col md={6}>
                            <Form.Group className="mb-3">
                                ...
                                <Form.Control placeholder="Seats" type="number"
                    ...
                    <Form.Group className="mb-3">
                        ...
                        <Form.Control placeholder="e.g. City Center - Campus"
